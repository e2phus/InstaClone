...
class ChatController: UICollectionViewController {
    ...
    private lazy var customInputView: CommentInputTextView = {
        let frame = CGRect(x: 0, y: 0, width: view.frame.width, height: 50)
        let iv = CommentInputTextView(config: .messages, frame: frame)
        iv.delegate = self
        return iv
    }()
...
extension ChatController: CommentInputTextViewDelegate {
    func inputView(_ inputView: CommentInputTextView, wantsToUploadComment text: String) {
