...
struct UserService {
    ...
    static func fetchUsers(forConfig config: UserFilterConfig, completion: @escaping([User]) -> Void) {
        ...
        case .followers(let uid):
            let reference = COLLECTION_FOLLOWERS.document(uid).collection("user-follwers")
            fetchUsers(fromCollection: reference, completion: completion)
            
        case .following(let uid):
            let reference = COLLECTION_FOLLOWING.document(uid).collection("user-following")
            fetchUsers(fromCollection: reference, completion: completion)
            
        case .likes(let postId):
            let reference = COLLECTION_POSTS.document(postId).collection("post-likes")
            fetchUsers(fromCollection: reference, completion: completion)
